...
- [ ] **Preconditions:** Fresh install, online mode, server ready.
- [ ] Launch app → see “Initial Setup Required” screen with “Begin Setup” button.
- [ ] Tap “Begin Setup” → Activation screen appears.
- [ ] Enter: Username `DiguwaSoft`, PIN `1366`.
- [ ] Activation UI shown → network call returns success with snapshot.
- [ ] Snapshot applied locally → log shows device binding.
- [ ] DB check: `librarians` row for DiguwaSoft has correct `device_id`.
- [ ] Redirected to `/auth/change-pin` (PIN change required).
- [ ] Enter old PIN `1366`, new PIN `2468`, confirm.
- [ ] Success message → redirected to Admin dashboard.
- [ ] DB check: `pin_salt` and `pin_hash` updated.
- [ ] Admin dashboard shows username and menu options.
- [ ] Log out → returned to `/auth/login`.

**Edge Cases:**

- [ ] Wrong PIN → “Activation Failed: Incorrect PIN”.
- [ ] No network → network error alert.
